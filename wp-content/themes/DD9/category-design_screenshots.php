                  ...
                  <li>
                    ...
                    <?php foreach($images as $image): ?>
                      <?php if($image->ID): $image_thumb = wp_get_attachment_image_src($image->ID, 'thumbnail');$image_full = wp_get_attachment_image_src($image->ID, 'large'); ?>
                        <a href="<?= $image_full[0] ?>" class="fancybox"><img src="<?= $image_thumb[0] ?>" width="234" alt="<?php the_title(); ?>"  title="<?= $image->post_excerpt ?>" /></a>
                      <?php endif; ?>
                    <?php endforeach; ?>
                    ...
                    <a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'twentyeleven' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark" class="thumbnail_title"><?php the_title(); ?></a>
